...
E2020A = E2020 %>%    
mutate(NumYear = as.numeric(as.character(unlist(E2020$Year))))
filter(Sector != "NA")
...
ggplot(sectormost, aes(YearNum,MmtCO2e, color = Sector))+
    geom_point()
...
```{r}
BySourceTotal = E2020A %>% 
    filter(Sector == "Total by Fuel" & Source != "Other" & Source != "Total")

BySourceTotal2019 = E2020A %>% 
    filter(Sector == "Total by Fuel" & Source != "Other" & Source != "Total" & NumericYear == 2019)

ggplot(BySourceTotal2019, aes(Source,MmtCO2e, fill = Source))+
    geom_col()

ggplot(BySourceTotal, aes(NumericYear, MmtCO2e, color = Source))+
    geom_line()+
...
#### 6. Which sector has the highest growth rate for each energy source?
```{r}
ratesector = C2020B %>%
...
ggplot(ratesector, aes(Sector, RateNumeric, fill = Source))+
    geom_col(position = "dodge")
...
#### 9.Compare projections in 2011 to actual measures in 2020. Compare slope and intercept of the trend lines.
```{r}
