...
C2011A = C2011 %>%
   ...
   values_drop_na = TRUE)
C2011B = C2011A %>%
mutate(Year = as.numeric(as.character(unlist(C2011A$Year))),
       GrowthRate = as.numeric(sub("%","",C2011A$`Growth Rate (2009-2035)`))) %>%
...
We choose these data because they clearly presents the different meausres for emission and consumption of energy with labeled year, sector and source. Thus, in order to see whether the US is doing well in reducing emissions and consumption and areas where we need to focus on improvement, we need to use these information to analyze and discuss about the energy performance in the US. The energy used in the dataset are measure by MmtCO2e (Million Metric Ton CO2) which is a common unit for measuring the energy emission and consumption. These datasets divided the energy by its sector and source which makes the anlysis easier because we can look at different energy sector and source separately to see which of them contribute the most in the increasing of emission and consumption of energy and which of them reduced the most. Also, we choose to use the data within a 10-year period because not only the common energy source and how energy is used are similar within 10 year, but also because we can track the changes in different variables closely in a shorter period and time and compare the actual changes in 10 years with our prediction from the previous dataset.
...
```
```{r}
renew_2011=ggplot(C2011Renew, aes(RenewableResource, energysum, fill = Sector))+
    geom_col(position = "dodge")+
    ggtitle("Sector Uses the Most Renewables",
            subtitle = "Based on 2011 Data")+
     ylab("Consumptionr(quads)")+
    xlab("Renewable")+
    theme(plot.margin=unit(c(1,1,-0.5,1),"cm"))+
    scale_y_log10() 
renew_2015=ggplot(C2015Renew, aes(RenewableResource, energysum, fill = Sector))+
    geom_col(position = "dodge") +
    ...
    theme(plot.margin=unit(c(1,1,-0.5,1),"cm"))+
    scale_y_log10() 
renew_2020 = ggplot(C2020Renew, aes(RenewableResource, energysum, fill = Sector))+
    geom_col(position = "dodge")+
    ggtitle("Which Sector Uses the Most Renewables?",
...
grid.arrange(renew_2011, renew_2015, renew_2020,nrow = 3)
```

#### 5.Which energy source is utilized the most in each sector?
```{r}
sector_most_2011 <- C2011B %>%
    filter(!str_detect(Source, "Total")) %>%
    filter(Year == "2011") %>%
    filter(!str_detect(Source, "Energy Use & Related Statistics") & Sector != "NA") %>%
    group_by(Sector) %>%
...
```
```{r}
ggplot(ratesector, aes(Sector, GrowthRate, fill= Source) ) +
    geom_col()+
    facet_wrap( ~ Year, nrow = 1) + 
    theme(legend.position = "none") +
