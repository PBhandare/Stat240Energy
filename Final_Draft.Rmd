...
# Introduction
  ...
  We were interested in looking at an environmentally related issue as this topici is currently a topic of debate. It is important to drive policy with a data driven factual approach, so we are using this report to visually represent United States energy consumption and Green House Gas Emissions.We are using energy statistics from the Annual Energy Outlook (AEO) report which is published each year by the the US Energy Information Administration. We chose to look at the AEO reports published in 2011, 2015 and 2020. We chose these years  because they represent 5 year benchmarks that cover the last decade. 

  For this report we hope to not only visually explain the statistics from energy data but also to show how we are doing as a country in the battle to  reducing emissions and consumption overall.These reports include actual measure from the year prior to their publishing and also projections for 20-30 years into the future past the report date. 
  
  We believe there are interesting relationships between variables to be uncoveered through visual analysis and modeling. We are interested in relationships between the variables such as: Industry Sector, Fuel Source, Emissions, and Consumption metircs within each of the data sets as well as comparing the variables across the separate years to look for trends. We will also compare  the prior years (2011 & 2015) projections to the observed measures from the 2020 report.We hope to use this analysis to locate key areas where the US is doing well in reducing emissions and consumption and areas where we need to focus on improvement.  
...
## **(11) Compare linear models for 2020-2030 time span between the 2011 and 2015 outlook data. Are the slopes significantly different?** 
...
sum_lm_2011 = lm(MmtCO2e~Year, data = lm2011)
co_fit = coef(sum_lm_2011)
xyear <- 2030
ypred <- co_fit[1] + co_fit[2]*xyear
lm_11 = ggplot(lm2011, aes(x=Year,y=MmtCO2e)) +
  geom_point() +
  geom_abline(aes(slope = co_fit[2], intercept=co_fit[1]), color="blue") +
  geom_point(aes(x = xyear, y=ypred), color = "red") +
  xlab("Year") +
  ylab("Energy Emission (MmtCO2e)") +
  ggtitle("2010 Emission")+
  scale_x_continuous(breaks = c(2020,2025,2030))

sum_lm_2015 = lm(MmtCO2e~Year, data = lm2015)
co_fit_2015 = coef(sum_lm_2015)
xyear_2015 <- 2030
ypred_2015 <- co_fit_2015[1] + co_fit_2015[2]*xyear_2015 
lm_15 = ggplot(lm2015, aes(x=Year,y=MmtCO2e)) +
  geom_point() +
  geom_abline(aes(slope = co_fit_2015[2], intercept=co_fit_2015[1]), color="blue") +
  geom_point(aes(x = xyear_2015, y=ypred_2015), color = "red") +
  xlab("Year") +
  ylab("Energy Emission (MmtCO2e)") +
  ggtitle("2015 Emission")+
  scale_x_continuous(breaks = c(2020,2025,2030))


sum_lm_2020 = lm(MmtCO2e~Year, data = lm2020)
co_fit_2020 = coef(sum_lm_2020)
xyear_2020 <- 2030
ypred_2020 <- co_fit_2020[1] + co_fit_2020[2]*xyear_2020 
lm_20 = ggplot(lm2020, aes(x=Year,y=MmtCO2e)) +
  geom_point() +
  geom_abline(aes(slope = co_fit_2020[2], intercept=co_fit_2020[1]), color="blue") +
  geom_point(aes(x = xyear_2020, y=ypred_2020), color = "red") +
  xlab("Year") +
  ylab("Energy Emission (MmtCO2e)") +
  ggtitle("2020 Emissionl")+
  scale_x_continuous(breaks = c(2020,2025,2030))
...
geom_point()+
  ...
  geom_vline(xintercept = 2030, color="red", linetype="dashed")+
