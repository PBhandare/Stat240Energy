...
# Introduction
  ...
  There are 3 focuses for our study, the first is how the most used energy source and sector changes throughout the decade and the reason behind the changes. As the technology and concept of environmental friendliness, we think the main energy source used in emission and consumption will become cleaner and more efficient. The second is how the total consumption and the total emission changed throughout the decades. We think that the efficiency of the energy use will be better and the total consumption and emission will decrease or become more environmentally friendly. The third is that how does the prediction based on the previous dataset compare to the later dataset as we are using the datas from a 10-year period. We think that some of the predictions will have major changes because of the changes in main energy sources and other factors. 
...
```{r,echo = FALSE}
fuel2011 = ggplot(BySourceTotal2011, aes(x= Source, y = MmtCO2e, fill = Source))+
    geom_col()+
    ggtitle("Total Emmisions By Fuel Source: 2010")+
    ylab("Emissions(MmtCO2e)")+
    xlab("Fuel Source")
fuel2015 = ggplot(BySourceTotal2015, aes(Source,MmtCO2e, fill = Source))+
    geom_col() +
    ggtitle("Total Emmisions By Fuel Source: 2014")+
    ylab("Emissions(MmtCO2e)")+
    xlab("Fuel Source")

fuel2020 = ggplot(BySourceTotal2020, aes(Source,MmtCO2e, fill = Source))+
    geom_col()+
    ggtitle("Total Emmisions By Fuel Source: 2019")+
    ylab("Emissions(MmtCO2e)")+
    xlab("Fuel Source")

grid.arrange(fuel2011,fuel2015,fuel2020)
...
```
 - The p value is 2.720731e-064 (less than 0.05 (typically ≤ 0.05)) is statistically significant. It indicates strong evidence against the null hypothesis, as there is less than a 5% probability the null is correct (and the results are random). Therefore, we reject the null hypothesis, and accept the alternative hypothesis of the mean use of petroleum is different from 2010-2020 period to 2020-2030 period. Looking at the mean of two dataset, we can see a clear increase in usage of Petroleum. 
...
```
 - The p value is 0.0005189996 (less than 0.05 (typically ≤ 0.05)) is statistically significant. It indicates strong evidence against the null hypothesis, as there is less than a 5% probability the null is correct (and the results are random). Therefore, we reject the null hypothesis, and accept the alternative hypothesis of the mean use of Coal  is different from 2010-2020 period to 2020-2030 period. 
 - Looking at the mean of two dataset, we can see a clear decline in usage of Coal. As renewable and other more efficient energy source being used, from the graph in previous question, we can seethe use of coal decline which contributing much lesser in energy emission. This is a good news because the coal-fired power plants release more greenhouse gases per unit of energy produced than any other electricity source (according to https://www.greenamerica.org/fight-dirty-energy/amazon-build-cleaner-cloud/coal-why-it-dirty).
...
### Discussion
 ...
 - Based on the data analysis, we can see some major changes in the use of energy source throughout this 10-year period. The first change we noticed is that the proportion of coal usage declined drastically from 2010 to 2020 data. Because of the decline in usage of coal, the emission of the industrial and electric power sector declined as well. We also compute a hypothesis test to see whether the proportion of the usage of petroleum and coal changes. This is a very good sign because coal combustion releases the greenhouse gases carbon dioxide (CO2) and nitrous oxide (N2O) during combustion which will significantly damaged our environment. And now we are mainly using cleaner and more efficient energy such as hydrogen and petroleum which will produce lesser energy emission. This matches with the improvement of energy efficiency we see looking at the total consumption and emission together within this 10-year period. 
